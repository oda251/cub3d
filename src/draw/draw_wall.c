...
void	update_radian(t_data *data)
{
...
void	draw_wall_utils(t_data *data)
{
	int		x;
	int		y;
	t_rgbo	color;
...
void	draw_wall(t_data *data)
{
	...
	while (x <= WIN_WIDTH)
	{
		data->wall->wall_height[x] = (int)((WIN_HEIGHT
					/ (data->wall->ray_length[x] * 8 / 7))); // to do 調節すること
		x++;
	}
	draw_wall_utils(data);//ここから&rayがないことに対応する
}
