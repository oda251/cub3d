...
static t_vector	calculate_side_dist(t_vector ray_dir, t_vector map_check,
		t_vector delta_dist, t_player *player)
{
	t_vector	side_dist;
...
static void	perform_dda(t_dda_data *data, double angle)
{
	...
	while (!hit)
	{
		...
		{
			hit = 1;
			if (data->ray.collisions_dir == EAST
				|| data->ray.collisions_dir == WEST)
			{
				data->ray.ray_length = (data->map_check.x
						- data->player->position.x + (1 - data->step.x) / 2)
					/ data->ray.direction.x;
				data->ray.wall_pos = fmod(data->player->position.x
						+ data->ray.ray_length * cos(angle), 1.0);
			}
			else
			{
				data->ray.ray_length = (data->map_check.y
						- data->player->position.y + (1 - data->step.y) / 2)
					/ data->ray.direction.y;
				data->ray.wall_pos = fmod(data->player->position.y
						+ data->ray.ray_length * sin(angle), 1.0);
			}
		}
	}
}
...
t_ray	cast_ray(t_player *player, char **map, double angle)
{
	...
	data.side_dist = calculate_side_dist(ray_dir, data.map_check,
			data.delta_dist, player);
	// data.ray = ray;
	data.player = player;
	data.map = map;
