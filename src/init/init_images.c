...
{
	data->wall = calloc_or_exit(sizeof(t_wall), 1);
	data->wall->no = calloc_or_exit(sizeof(t_wall_info), 1);
	data->wall->so = calloc_or_exit(sizeof(t_wall_info), 1);
	data->wall->we = calloc_or_exit(sizeof(t_wall_info), 1);
	data->wall->ea = calloc_or_exit(sizeof(t_wall_info), 1);
	// 以下のWIN_WIDTHに+1しました heap buffer overflowを防ぐため 原理はわかってない
	data->wall->collisions_dir = calloc_or_exit(WIN_WIDTH + 1,
			sizeof(t_direction_draw));
	data->wall->ray_length = calloc_or_exit(WIN_WIDTH + 1, sizeof(double));
	data->wall->wall_pos = calloc_or_exit(WIN_WIDTH + 1, sizeof(double));
	data->wall->wall_height = calloc_or_exit(WIN_WIDTH + 1, sizeof(int));
}
